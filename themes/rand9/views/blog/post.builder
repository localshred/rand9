...
xml.feed :'xml:lang' => 'en-US', :xmlns => 'http://www.w3.org/2005/Atom' do
  ...
  xml.link :type => 'text/html', :href => "http://#{hostname}/#{Marley::Configuration.blog.pathname}/#{@post.id}", :rel => 'alternate'
  ...
  xml.subtitle  "#{hostname}/#{Marley::Configuration.blog.pathname}/#{@post.id}"
  ...
  @post.comments.each_with_index do |comment, index|
    xml.entry do |entry|
      ...
      entry.link :type => 'text/html', :href => "http://#{hostname}/#{Marley::Configuration.blog.pathname}/#{@post.id}#comment_#{index}", :rel => 'alternate'
